...
/** The client's own triangle rasterizer, ported line for line from Pix3D.gouraudTriangle/
 * Pix3D.gouraudRaster (see the Follower Buddy plugin's own com.follower.ui.GouraudRasterizer, the
 * source of this port -- https://github.com/MikeSpatol/follower-buddy).
 * <p>
 * This replaces a float barycentric fill that sampled pixel centres. On triangles a few pixels
 * across -- the pupils of a chathead's eyes -- the two disagree visibly: a sub-pixel face can miss
 * every pixel-centre sample and vanish entirely, and adjacent faces can leave one-pixel cracks
 * along shared edges. The game's rasterizer is an integer scanline walker with its own fill
 * convention; only running the same algorithm produces the same pixels.
 * <p>
 * x steps are 16.16 fixed point ((dx &lt;&lt; 16) / dy), colour steps 15-bit ((dc &lt;&lt; 15) /
 * dy); scanline endpoints are handed to the raster as x &gt;&gt; 16 / colour &gt;&gt; 7, and the
 * raster indexes the colour table with colour &gt;&gt; 8 per pixel. Colours interpolate as PACKED
 * palette indices and resolve through GameColourTable, the same gamma table the game uses. Only
 * the paths ChatheadRenderer actually exercises are ported: high detail, opaque, with horizontal
 * clipping always on. */
